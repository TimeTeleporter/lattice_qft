...
impl MarkovWrapper {
    ...
    fn binning(self) -> Self {
        let new_set: Vec<f64> = Vec::with_capacity(self.0.len() / 2 + 1);

        for index in self.0.len() {
            let 
        }
    }
}
...
fn calculate_bin_var(
    ...
    let mut temp_data: MarkovWrapper = data.clone();

    let mut index: u32 = 0;
    while 2_usize.pow(index) < data.0.len() {
        index += 1;
